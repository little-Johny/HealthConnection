...
class ScheduleService {
    ...
    async create(data) {
        ...
        try {
            ...
            if (error.name === 'SequelizeUniqueConstraintError') {
                throw boom.conflict(
                    `Ya hay un horario definido para ${data.dayOfWeek}`,
                );
            }
            throw error;
        }
    ...
    async blockSchedule(doctorId, data, transactionOptions = {}) {
        ...
        const dateInBogota = DateTime.fromISO(data.date, {
            zone: 'America/Bogota',
        });
        ...
        if (nowInBogota.startOf('day') > dateInBogota.startOf('day')) {
            throw boom.badRequest(
                `No es posible hacer un bloqueo para la fecha ${dateInBogota.toFormat('yyyy-MM-dd')}, porque no se puede bloquear un día pasado.`,
            );
        }
        ...
        if (blockDateTime < nowInBogota) {
            throw boom.badRequest(
                `No es posible bloquear en el pasado. La hora ingresada (${data.startTime}) ya pasó.`,
            );
        }

        // Convertir startTime a DateTime y restarle 10 minutos
        const startTimeInFormat = DateTime.fromFormat(data.startTime, 'HH:mm:ss', {
            zone: 'America/Bogota',
        });
        ...
        if (!schedule) {
            throw boom.notFound(
                `El doctor ${doctorId} no tiene un horario para el día ${dayOfWeek}`,
            );
        }
        ...
        if (
            ...
            throw boom.badRequest(
                'El intervalo seleccionado está fuera del horario disponible del doctor',
            );
        }
        ...
        if (existingBlock) {
            throw boom.conflict(
                `Ya existe un bloqueo en la franja horaria ${data.startTime} - ${data.endTime}`,
            );
        }
    ...
    async findSchedule(state, doctorId, date) {
        ...
        if (!schedule) {
            throw boom.notFound(
                `El doctor ${doctorId} no tiene horario para el día ${dayOfWeek}`,
            );
        }
        ...
        let currentTime = DateTime.fromFormat(schedule.startTime, 'HH:mm:ss', {
            zone: 'America/Bogota',
        });

        const endTime = DateTime.fromFormat(schedule.endTime, 'HH:mm:ss', {
            zone: 'America/Bogota',
        });
        ...
        while (currentTime < endTime) {
            ...
            const isBlocked = blocks.some((block) => {
                const blockStart = DateTime.fromFormat(block.startTime, 'HH:mm:ss', {
                    zone: 'America/Bogota',
                });
                const blockEnd = DateTime.fromFormat(block.endTime, 'HH:mm:ss', {
                    zone: 'America/Bogota',
                });
