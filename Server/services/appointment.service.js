...
class AppointmentService {
    async create(data) {
        ...
        try {
            ...
            const newAppointment = await models.Appointment.create(appointmentData, {
                transaction,
            });
    ...
    async find(query) {
        ...
        const {
            patient, doctor, speciality, ...filters
        } = query;
        ...
        if (speciality) {
            const foundSpec = await models.Speciality.findOne({
        ...
        options.include.push(
            {
                ...
                as: 'patient',
                include: [
                    { model: models.User, as: 'user', attributes: ['name', 'lastName'] },
                ],
            },
            {
                model: models.Doctor,
        ...
        if (appointments.length === 0) {
            throw boom.notFound(
                `No se encontró ninguna cita ${filterMessages.join(', ')}`,
            );
        }
        ...
        try {
            ...
            if (appointment.status !== 'pending') {
                throw boom.badRequest(
                    'Solo se pueden actualizar citas con estado pendiente.',
                );
            }
            ...
            const isTimeChanged = changes.date
        || changes.startTime
        || changes.endTime
        || changes.doctorId;
            ...
            if (
                changes.specialityId
        && changes.specialityId !== appointment.specialityId
            ) {
                const speciality = await specialityService.findOne(
                    changes.specialityId,
                );
            ...
            const updatedAppointment = await appointment.update(updatedFields, {
                transaction,
            });
            await transaction.commit();
    ...
    async delete(id) {
        ...
        || appointment.status === 'completed'
                ...
                throw boom.badRequest(
                    'Esta cita no se puede eliminar porque ya fue cancelada o completada',
                );
            }
