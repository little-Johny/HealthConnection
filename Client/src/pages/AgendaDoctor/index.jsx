...
const AgendaDoctor = () => {
    ...
    const { userId } = useAuth(); // Suponiendo que doctorId se obtiene desde el hook useAuth
    const [citas, setCitas] = useState([]);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        if (!userId) {
            toast.error("No se pudo obtener el ID del doctor.");
            return;
        }

        const fetchCitas = async () => {
            try {
                const response = await axios.get(`http://localhost:3000/health_connection/v1/cita/doctorCita/${userId}`);
    ...
    return (
        <div className="bg-gray-100 min-h-screen">
            {/* Encabezado */}
            <header className="bg-orange-500 p-6 shadow-lg">
            ...
            <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md mt-6">
                {citas.length === 0 ? (
                    <p className="text-center text-gray-600">No hay citas disponibles para este doctor.</p>
                ) : (
                    <div className="space-y-6">
                        {citas.map((cita) => (
                            <div key={cita.id} className="border p-4 rounded-lg shadow-sm hover:shadow-lg transition-all duration-200">
                                <h3 className="text-xl font-medium text-gray-800">{`${cita.paciente_nombre} ${cita.paciente_apellido}`}</h3>
