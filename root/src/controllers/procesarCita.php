...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $sql = $database->prepare("INSERT INTO Cita (id_paciente, id_doctor, id_Administrativos, fecha, hora, tipo_cita, costo, especialidad, requiere_autorizacion) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $result = $sql->execute([$doc_paciente, $preferred_doctor, $id_administrativo, $date, $time, $type, $costo, $specialty, $autorization]);
