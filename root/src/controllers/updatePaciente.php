...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $sentencia = $database->prepare("
        UPDATE Paciente
        SET nombre = ?, apellido = ?, email = ?, telefono = ?, direccion = ?, contraseña = ?, id_afiliacion = ?, foto_perfil = ?
        WHERE numero_documento = ?
    ");

    // Ejecutar la sentencia de actualización
    $resultado = $sentencia->execute([$nombre, $apellido, $email, $telefono, $id_direccion, $contraseña, $id_afiliacion, $fotoPerfil, $numero_documento]);

    if ($resultado) {
        header("Location:../views/admin/verPacientes.php");
    } else {
